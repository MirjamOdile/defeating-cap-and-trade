...
hearings <- committee_members_matched %>%
  ...
  mutate(inc_total_fossilelectric = inc_total_fossil + inc_total_electric,
         per_inc_fossilelectric = per_inc_fossil + per_inc_electric) %>%
  group_by(committee) %>%
  # Extract group means
  mutate(across(c(per_inc_fossilelectric, per_inc_alternate, per_emp_fossil),
                ~ mean(.x, na.rm=T), .names = "{.col}_AVE")) %>%
  # Centering within clusters (CWC)
  mutate(across(c(per_inc_fossilelectric, per_inc_alternate, per_emp_fossil),
                ~ .x - mean(.x, na.rm=T), .names = "{.col}_CWC")) %>%
  # Scale CWC variables
  mutate(across(per_inc_fossilelectric_CWC:per_emp_fossil_CWC,
                scale, .names = "{.col}_SC")) %>%
  mutate(across(per_inc_fossilelectric_CWC_SC:per_emp_fossil_CWC_SC,
                ~replace_na(.,0))) %>%
  ungroup() %>% 
  # Scale AVE variables
  mutate(across(per_inc_fossilelectric_AVE:per_emp_fossil_AVE,
                scale, .names = "{.col}_SC"))
...
loo(con0, con1, con2, con3, moment_match = F)
